...
import static xyz.rongmario.dexterity.DexterityData.MINING_SPEED_PERK_UUID;
    ...
    public static GenericPerk getGenericBlockBreakPerk(String name, AbstractToolSkill<?, ?> skill) {
        return PerkBuilder.<GenericPerk>of()
                ...
                .start((player, world, stack) -> {
                    ...
                    player.getAttributeInstance(DexterityEntityAttributes.MINING_SPEED_PERK).addTemporaryModifier(new EntityAttributeModifier(MINING_SPEED_PERK_UUID, "Bonus Dexterity Mining Speed", speed, EntityAttributeModifier.Operation.ADDITION));
                    return ActionResult.PASS;
                })
                .end((player, world, stack) -> player.getAttributeInstance(DexterityEntityAttributes.MINING_SPEED_PERK).removeModifier(MINING_SPEED_PERK_UUID))
